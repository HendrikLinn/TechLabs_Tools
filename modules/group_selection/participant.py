...
parent_dir = os.path.abspath(os.path.join(os.path.dirname("utilities.py"),"..", "modules"))
...
def read_csv(file_path: str, delimiter: str = ",") -> List[Participant]:
    ...
    Returns:
        List[Participant]: List containing the participtans as objects of the ``Participants`` dataclass.
    """
...
def read_excel(file_path: str) -> List[Participant]:
    ...
    Returns:
        List[Participant]: List containin the participants as objects of the ``Participant`` dataclass.
    """
...
def create_name_id_map(data: pd.DataFrame, name_columns: List[str], save_map: bool = True) -> dict:
    ...
    Args:
        ...
        save_map (bool, optional): If True, the mapping is saved to a json file in the config directory. Defaults to True.
    ...
    name_id_map = {}
    for name, id in zip(part_names, range(1, len(part_names) + 1)):
        name_id_map[name] = id
